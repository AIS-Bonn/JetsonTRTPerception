...
struct Args
{
    ...
    int32_t batch{1};
    std::vector<std::string> dataDirs, fileNames;
    std::string onnxFilePath_segm;
    std::string saveEngine;
    std::string loadEngine;
    bool useILoop{false};
    bool aspectRatio16by9{false};
...
inline bool parseArgs(Args& args, int32_t argc, char* argv[])
{
    ...
    {
        int32_t arg;
        static struct option long_options[] = {{"help", no_argument, 0, 'h'}, {"datadir", required_argument, 0, 'd'}, {"img", required_argument, 0, 'j'}, {"num_classes", required_argument, 0, 'n'}, {"top_k", required_argument, 0, 't'},
            {"int8", no_argument, 0, 'i'}, {"fp16", no_argument, 0, 'f'}, {"useILoop", no_argument, 0, 'l'}, {"aspectRatio16by9", no_argument, 0, 'w'}, {"model_path_segm", required_argument, 0, 'm'},
        ...
        case 'j':
            ...
            {
                std::cerr << "ERROR: --img requires option argument" << std::endl;
                return false;
            }
            break;
        case 's':
        ...
        case 'n':
            if (optarg)
            {
                args.num_classes = std::stoi(optarg);
            }
            break;
        case 't':
            if (optarg)
